...
public class MainWindow
{
    ...
    public MainWindow(KeyStoreManager keyStoreManager, AppHandler aph, boolean forSigning) throws Exception
    {        
    ...
    JFrame getMainFrame() throws Exception
    {
        if (mainFrame == null)
        {
            ...
            mainFrame.setTitle(LabelManager.get("WINDOW_TITLE"));
...
//            mainFrame.setLocation(_width / 2 - 582 / 2, _height / 2 - 518 / 2);
            ...
            if (devices.length > 1 && _aph.getScreen() > 0 && _aph.getScreen() < devices.length)
            {
            	GraphicsDevice device = devices[_aph.getScreen()];
            	
    			int _height = device.getDefaultConfiguration().getBounds().height;
            	int _width = device.getDefaultConfiguration().getBounds().width;
            	
            	int locationX = device.getDefaultConfiguration().getBounds().x + _width / 2 - 582 / 2;
            	int locationY = device.getDefaultConfiguration().getBounds().y + _height / 2 - 518 / 2;
    ...
    private JPanel getMainContentPane() throws Exception
    {
        if (mainContentPane == null)
        {
            ...
            {
            	mainContentPane.add(getSelectButton(), null);
            }
...
//    }
    ...
    private JButton getSelectButton()
    {
        if (SignButton == null)
        {
            ...
            SignButton.setText(LabelManager.get("BUTTON_SELECT"));
            SignButton.setEnabled(false);
            SignButton.addActionListener(_evthandler.getdoSelectActionListener());
        }
        return SignButton;
    }
    ...
    private JMenu getHelpAboutMenu()
    {
        if (helpAboutMenu == null)
        {
            ...
            helpAboutMenu.setPreferredSize(new Dimension(51, 20));
    ...
    protected JTree getJTree() throws Exception
    {
        ...
        if (jTree == null)
        {
            ...
            DefaultMutableTreeNode dmf = jbt.build(_aph, _aph.getAcceptedIssuers(), _aph.getCertificateHashFilter(), _aph.getCertificateSerialFilter(), this.keyStoreManager.getKeyStoreTable());
    ...
    public JButton getCurrentSignButton()
    {
