...
public class X509CertificateHandler
{
    ...
    public X509CertificateHandler(X509Certificate xcer, String alias, IKeyStore iksh) throws CertificateParsingException
    {
    	_iksh = iksh;
    	initialize(xcer, alias, _iksh.getProvider(), _iksh.getName(), _iksh.getTokenName());
    }

    public X509CertificateHandler(X509Certificate xcer, String alias, Provider provider, SupportedKeystore storeName, String tokenName) throws CertificateParsingException
    {
        _iksh = null;
        initialize(xcer, alias, provider, storeName, tokenName);
    }

    public void initialize(X509Certificate xcer, String alias, Provider provider, SupportedKeystore storeName, String tokenName) throws CertificateParsingException
    {
        String auxStr;
        int auxIdx;
        ...
        auxIdx = _SubjectDN.indexOf("CN=");
        
        if (auxIdx != -1)
        {
	        auxStr = _SubjectDN.substring(auxIdx);
	        auxStr = auxStr.replaceFirst("CN=", "");
	        ...
	        if (auxStr.indexOf("=") > -1)
	        {
	        	...
	        	if (auxStr.lastIndexOf(" ") > 0)
	        	{
	        		auxStr = auxStr.substring(0, auxStr.lastIndexOf(" ") - 1);
	        	}
	        }
    ...
    public boolean isCKCProvider()
    {
        System.out.println("STORE: " + getStoreName());
        if (getStoreName().equals(SupportedKeystore.CKC))
        {
            return true;
        }
        else
