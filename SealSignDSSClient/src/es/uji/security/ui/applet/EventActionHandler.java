...
public class EventActionHandler
{
    ...
    /**
     * Creates the certificate selection action listener
     * 
     * @return res The action listener on a signature creation event
     */
    public java.awt.event.ActionListener getdoSelectActionListener()
    {
        ActionListener res = new java.awt.event.ActionListener()
        {
            public void actionPerformed(java.awt.event.ActionEvent e)
            {
            	aph.getMainWindow().getShowSignatureCheckBox().setVisible(false);
            	
            	try
            	{
            		X509CertificateHandler selectedNode = (X509CertificateHandler)((DefaultMutableTreeNode)mw.jTree.getLastSelectedPathComponent()).getUserObject();
            		
            		String certificateBase64 = Base64.encode(selectedNode.getCertificate().getEncoded());
            		
            		aph.callJavaScriptCallbackFunction(aph.getJsSelectOk(), new String[] { certificateBase64 });
            	}
            	catch (Exception ex)
            	{
            		ex.printStackTrace();
    ...
    public TreeSelectionListener getJTreeSectionListener()
    {
        TreeSelectionListener tsl = new TreeSelectionListener()
        {
            public void valueChanged(TreeSelectionEvent e)
            {
                ...
                JButton auxCurrentSignButton = mw.getCurrentSignButton();

                try
                {
                    X509CertificateHandler selectedNode = (X509CertificateHandler) ((DefaultMutableTreeNode) mw.jTree.getLastSelectedPathComponent()).getUserObject();
//                    boolean pk11 = (selectedNode.isPKCS11Provider() || selectedNode.isClauerProvider());
                    boolean pk11 = (selectedNode.isPKCS11Provider());
                    ...
                    String desc = selectedNode.getTokenName();
                    if (desc != null)
                    {
                    	desc = desc.trim().replace(' ', '_');
                    	String txtShow = LabelManager.get(desc);
                    ...
                    mw.getInformationLabelField().setText(LabelManager.get("SELECT_A_CERTIFICATE"));
                    
                    auxCurrentSignButton.setEnabled(true);
                }
                catch (Exception exc)
                {
                    ...
                    auxCurrentSignButton.setEnabled(false);
                }
            }
        };
